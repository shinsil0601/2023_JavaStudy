...
public class Ex02_Canvas extends JFrame{
	...
	private class CanvasTest extends Canvas{
		...
		public void paint(Graphics g) {
			...
			//선그리기 : drawLine(시작위치x, 시작위치y, 끝위치x, 끝위치y)
			g.drawLine(10, 10, 110, 10);  //가로직선
			g.drawLine(10, 10, 10, 110);  //세로직선
			...
			//사각형: drawRect(시작위치x, 시작위치y, 넓이, 높이)
			...
			//둥근모서리 사각형 : drawRoundRect(시작위치x, 시작위치Y, 높이, 넓이, 호넓이, 호높이)
			//둥근모서리 채워진 사각형 : fillRoundRect(시작위치x, 시작위치Y, 높이, 넓이, 호넓이, 호높이)
			g.drawRoundRect(340, 10, 100, 100, 0, 0);  //0,0 사각형
			g.setColor(Color.RED);
			g.fillRoundRect(350, 20, 80, 80, 30, 30);  //30,30(둥근정도 100으로가면 원)
			g.setColor(Color.YELLOW);
			g.drawRoundRect(360, 30, 60, 60, 30, 30);
			g.setColor(Color.GREEN);
			g.fillRoundRect(370, 40, 40, 40, 100, 100);  //100,100 원
			
			//원 drawOval(시작위치, 넓이, 높이)
			//채워진 원: fillOval(시작위치, 넓이, 높이)
			//색지정: Color(0~255(R), 0~255(G), 0~255(B))
			g.setColor(new Color(0,0,0));   //검정
			g.drawOval(10, 120, 100, 100);
			g.setColor(new Color(255,255,255));
			g.fillOval(20, 130, 80, 80);
			
			//글자 : drawString("글자", 시작위치(글자 밑부분)
