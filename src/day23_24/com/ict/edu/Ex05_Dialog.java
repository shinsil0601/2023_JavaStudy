	...
	public Ex05_Dialog() {
		...
		jb1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(jp, "방가방가", "MASSAGE 창", JOptionPane.PLAIN_MESSAGE);
			}
		});
		jb2.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 확인 다이얼로그: 정보를 알리고 사용자 반응의 결과를 가져온다.(확인(0), 아니오(1), 취소(2))
				// JOptionPane.showConfirmDialog(parentComponent, message);
				//  int res = JOptionPane.showConfirmDialog(jp, "정말 삭제할까요");  //정보를 받을 변수 선언
		...
		jb3.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 입력 다이얼로그: 사용자에게 정보를 받는 다이얼로그 (js(자바스크립트에서 prompt() => 거의 사용안함)
				// String msg = JOptionPane.showInputDialog(parentComponent, message); 
				// String msg = JOptionPane.showInputDialog(jp, "나이: "); //정보를 입력받을 변수 선언
		...
		jb4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// JOptionPane.showOptionDialog(parentComponent, message, title, optionType, messageType, null, null, null);
				 int res = JOptionPane.showOptionDialog(jp, "방가방가", "사용자 다이얼로그", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
				// System.out.println(res);
